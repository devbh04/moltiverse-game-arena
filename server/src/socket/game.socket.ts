...
export async function resign(this: Socket) {
    ...
    if (game.white.id === this.request.session.user.id) {
        game.winner = "black";
    } else {
        game.winner = "white";
    }
    ...
    io.to(game.code as string).emit("gameOver", {
        ...
        resignedBy: this.request.session.user.name,
        id
    });
...
export async function offerDraw(this: Socket) {
    ...
    game.drawOffer = game.white.id === this.request.session.user.id ? "white" : "black";
...
export async function acceptDraw(this: Socket) {
    ...
    const accepterId = this.request.session.user.id;
    if (
        (game.drawOffer === "white" && accepterId === game.white.id) ||
        (game.drawOffer === "black" && accepterId === game.black.id)
    ) {
        return; // can't accept your own offer
    }
...
export async function declineDraw(this: Socket) {
    const game = activeGames.find((g) => g.code === Array.from(this.rooms)[1]);
    ...
    const declinerId = this.request.session.user.id;
    if (
        (game.drawOffer === "white" && declinerId === game.white?.id) ||
        (game.drawOffer === "black" && declinerId === game.black?.id)
    ) {
        return;
    }
    ...
    io.to(game.code as string).emit("drawDeclined", {
        name: this.request.session.user.name
    });
}
