...
export function initSocket(
    ...
    socket.on(
        ...
        ({
            ...
            if (reason === "resign") {
                m.message = `${resignedBy} has resigned. ${winnerName} (${winnerSide}) wins!`;
            ...
            actions.updateLobby({
                type: "updateLobby",
                payload: { endReason: reason, winner: winnerSide || "draw", id, drawOffer: undefined }
            });
            actions.addMessage(m);
        }
    );

    socket.on("drawOffered", ({ by, name }: { by: "white" | "black"; name: string }) => {
        actions.addMessage({
            author: { name: "server" },
            message: `${name} has offered a draw.`
        });
        actions.updateLobby({
            type: "updateLobby",
            payload: { drawOffer: by }
        });
    });

    socket.on("drawDeclined", ({ name }: { name: string }) => {
        actions.addMessage({
            author: { name: "server" },
            message: `${name} has declined the draw offer.`
        });
        actions.updateLobby({
            type: "updateLobby",
            payload: { drawOffer: undefined }
        });
    });
}
